...
Fragmented Daily Utilities: Users switch between multiple apps to pay bills, split costs with friends, recharge mobile accounts, and manage micro-investments.
Social Transaction Friction: Splitting bills (for rent, dining, or trips) remains manual and socially awkward. No seamless pipeline directly converts a calculated split into a validated UPI transfer inside a group context.
Complex Financial Products: Insurance and investment options remain inaccessible or overly complex for the average Indian smartphone user, who prefers low-ticket-size, highly intuitive financial instruments (e.g., "sachet" insurance or micro-investing in digital gold).
Merchant Verification Overhead: Small merchants (Kirana owners) lose valuable business time manually checking phone screens or SMS notifications to verify incoming customer payments.
...
Transaction Latency: Core UPI payment status resolution must occur in 
≤
2.5
 seconds
≤2.5 seconds from MPIN submission on a 3G/4G network.
High Availability: App backends must achieve 
99.99
%
99.99% uptime.
Peak Load Handling: Designed to scale dynamically to process up to 
25
,
000
 Transactions Per Second (TPS)
25,000 Transactions Per Second (TPS) during national festivals and sales events (e.g., Diwali).
...
Engagement	Monthly Active Users (MAU) / Daily Active Users (DAU)	Ratio of DAU/MAU 
≥
0.40
≥0.40
Payment Performance	Transaction Success Rate (TSR)	
≥
99.5
%
≥99.5% on Core UPI rails (excluding bank-side downtimes)
